  ...
  create or replace view `gold-north-america`.`gold_north_america`.`int_team_strength`
  OPTIONS()
  as with match_results as (
    select * from `gold-north-america`.`gold_north_america`.`stg_match_results`
...
with_rankings as (
    ...
    from normalised n
    left join `gold-north-america`.`gold_north_america`.`stg_fifa_rankings` f
        on n.team = f.team
),
