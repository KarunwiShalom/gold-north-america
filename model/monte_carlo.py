...
def simulate_match_knockout(lambda_home, lambda_away):
    ...
    If draw after 90 mins, go to penalties (50/50).
    """
...
def simulate_group_stage(fixtures_df):
    ...
    Returns standings for each group.
    """
    # Track points, GD, GF for each team
...
def get_group_standings(standings):
    ...
    Returns dict of group -> [1st, 2nd, 3rd, 4th]
    """
...
def get_best_third_place(standings, group_standings):
    """
    Get the 8 best third-placed teams across all 12 groups.
...
def build_r32_bracket(group_standings, best_third):
    """
    FIFA 2026 official bracket.
    Maps group winners (1st), runners-up (2nd), and 
    best third place teams to R32 matchups.
    """
    g = group_standings
    ...
    bracket = [
        ...
        # Match 3: 1A vs 2B
        (get('A', 0), get('B', 1)),
        # Match 4: 2A vs 2B — actually 1F vs 2C
        (get('F', 0), get('C', 1)),
        # Match 5: 2K vs 2L
        (get('K', 1), get('L', 1)),
        # Match 6: 1H vs 3CEFHI
        (get('H', 0), best_third[2]),
        # Match 7: 1D vs 3BEFIJ
        (get('D', 0), best_third[3]),
        # Match 8: 1G vs 2J
        (get('G', 0), get('J', 1)),
        # Match 9: 1C vs 2G (approx)
        (get('C', 0), get('G', 1)),
        # Match 10: 1B vs 2A
        (get('B', 0), get('A', 1)),
        # Match 11: 1J vs 3DEIJL
        (get('J', 0), best_third[4]),
        # Match 12: 1K vs 3EFGIJ
        (get('K', 0), best_third[5]),
        # Match 13: 1L vs 3AEHIJ
        (get('L', 0), best_third[6]),
        # Match 14: 1C vs 2D (approx)  
        (get('C', 0), get('D', 1)),
        # Match 15: 2E vs 2I
        (get('E', 1), get('I', 1)),
        # Match 16: 2F vs 2H
...
def simulate_knockout_round(bracket, team_lambdas):
    ...
    for home, away in bracket:
        ...
        # Lambda = attacker's attack * defender's defence * baseline
        baseline = 1.1
...
def build_team_lambdas(fixtures_df):
    """Build attack/defence lookup per team from fixture data."""
    team_lambdas = {}
    for _, row in fixtures_df.iterrows():
...
def simulate_tournament(fixtures_df, team_lambdas):
    """Simulate one full tournament. Returns the winner."""
...
def run_monte_carlo(fixtures_df, n=10000):
    """Run N tournament simulations and return probabilities."""
